...
export default function UsersTable() {
  ...
  const saveUser = async () => {
    try {
      const res = await fetch(`${API_URL}/api/users/${tempUser.id}`, {
        method: 'PUT',
        credentials: 'include',
              ...
              filteredUsers.map(user => (
                <tr key={user.id} className={editingUser === user.id ? 'editing' : ''}>
                  ...
                  <td className="actions-cell">
                    {editingUser === user.id ? (
                      <>
                        <button onClick={saveUser} className="save-btn" title="Save changes">💾</button>
                        <button onClick={cancelEditing} className="cancel-btn" title="Cancel">❌</button>
                      </>
                    ) : (
                      <>
                        <button onClick={() => startEditing(user)} className="edit-btn" title="Edit user">✏️</button>
                        <button onClick={() => deleteUser(user.id)} className="delete-btn" title="Delete user">🗑️</button>
                      </>
                    )}
                  </td>
                </tr>
